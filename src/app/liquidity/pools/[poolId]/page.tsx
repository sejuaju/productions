...
const TokenCard = ({ token }: { token: Token }) => (
    <div className="bg-[var(--hover)] dark:bg-[var(--bg-primary)] rounded-xl p-5 shadow-sm border border-[var(--card-border)]">
        <div className="flex items-center justify-between mb-4">
            ...
            <a href={`https://explorer.exatech.io/token/${token.token_address}`} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-500 hover:underline">
                View on Explorer
            </a>
        </div>
...
export default function PoolDetailPage() {
  ...
  const renderContent = () => {
    ...
    if (error || !pool) {
      return (
        <div className="text-center py-10">
          ...
          <Link href="/liquidity/pools" className="bg-[var(--primary)] text-white py-2 px-5 rounded-lg hover:bg-[var(--primary-dark)] transition font-semibold">
            Back to Pools
          </Link>
        </div>
      );
    ...
    return (
      <div className="card p-4 sm:p-6 max-w-7xl mx-auto shadow-lg dark:shadow-2xl dark:bg-[var(--bg-card)] border border-[var(--card-border)] rounded-2xl">
        <div className="flex justify-between items-center mb-6 px-2">
          ...
          <Link href="/liquidity/pools" className="p-2 rounded-full hover:bg-[var(--hover)] transition">
                ...
                {pool.liquidity_events.slice(0, 10).map((tx, idx) => (
                  <tr key={tx.tx_hash + idx} className="hover:bg-[var(--primary)]/10">
                    <td className="py-3 px-4">
                      <a href={`https://explorer.exatech.io/tx/${tx.tx_hash}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2">
      ...
      <div className="py-6">
        <div className="max-w-4xl mx-auto text-center mb-8">
          ...
          <p className="text-[var(--text-secondary)] max-w-2xl mx-auto">
            Review detailed statistics and recent transactions for this liquidity pool.
          </p>
        </div>
        {renderContent()}
      </div>
